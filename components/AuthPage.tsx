    ...
    <div className="h-screen w-screen bg-black flex flex-col items-center justify-center p-6 overflow-hidden relative">
      ...
      <div className="relative z-10 w-full max-w-sm space-y-12 text-center">
        <div className="space-y-4">
          <div className="inline-flex items-center justify-center w-28 h-28 rounded-[2.5rem] bg-white text-black font-black text-5xl shadow-[0_0_60px_rgba(255,255,255,0.2)] animate-pulse mb-4">
            ZX
          </div>
          <div className="space-y-2">
            <h1 className="text-4xl font-black tracking-tighter text-white">ZOHAIB X NO 18</h1>
            <p className="text-gray-500 text-[10px] font-black uppercase tracking-[0.4em] px-4">The People's AI • Unfiltered & Free</p>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="p-6 bg-white/5 border border-white/10 rounded-3xl backdrop-blur-md">
            <p className="text-gray-300 text-xs font-medium leading-relaxed italic">
              "Sab ke liye free hai bhai, enjoy karo!"
            </p>
          </div>
          ...
          <button 
            onClick={handleEnter}
            disabled={loading}
            className={`group w-full flex items-center justify-center gap-3 py-5 px-6 rounded-2xl font-black text-sm uppercase tracking-[0.25em] transition-all transform active:scale-95 ${loading ? 'bg-gray-800 text-gray-500' : 'bg-white text-black hover:bg-gray-100 hover:shadow-[0_0_30px_rgba(255,255,255,0.15)]'}`}
          >
            {loading ? (
        ...
        <div className="pt-20">
          <p className="text-[9px] text-gray-700 font-bold uppercase tracking-[0.5em] animate-pulse">Developed by Legend</p>
        </div>
      </div>
    </div>
  );
