  ...
  return (
    <div className={`flex h-screen w-full overflow-hidden transition-colors duration-300 ${theme === 'dark' ? 'bg-[#0d0d0d] text-gray-200' : 'bg-white text-gray-900'}`}>
      ...
      <div className="flex-1 flex flex-col h-full relative">
        <header className={`h-14 flex items-center justify-between px-4 md:px-6 border-b shrink-0 backdrop-blur-md z-10 transition-colors duration-300 ${theme === 'dark' ? 'border-white/10 bg-black/50' : 'border-black/5 bg-white/50'}`}>
          <button 
            onClick={() => setIsSidebarOpen(true)}
            className={`md:hidden p-2 rounded-lg transition-colors ${theme === 'dark' ? 'hover:bg-white/5' : 'hover:bg-black/5'}`}
          >
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="3" y1="12" x2="21" y2="12"></line><line x1="3" y1="6" x2="21" y2="6"></line><line x1="3" y1="18" x2="21" y2="18"></line></svg>
          </button>
          
          <div className="text-sm font-black tracking-tighter uppercase">ZOHAIB X NO 18</div>
